...
class Command(BaseCommand):
    def add_arguments(self, parser):
        parser.add_argument(
            ...
            dest="all",
            action="store_true",
            help="All feeds, need it or not (can be combined with a user)",
        ),
        parser.add_argument(
            "-s", "--silent", dest="silent", default=False, action="store_true", help="Inverse verbosity."
        ),
    ...
    def handle(self, *args, **options):
        ...
        if options["user"]:
            ...
            users = User.objects.filter(
                profile__last_seen_on__gte=datetime.datetime.now() - datetime.timedelta(days=options["days"])
            ).order_by("pk")
