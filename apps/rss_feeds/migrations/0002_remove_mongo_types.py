...
# Updated for pymongo 4.x compatibility
...
def remove_mongo_types(apps, schema_editor):
    ...
    collections = db.list_collection_names()  # Updated from collection_names()
    for collection_name in collections:
        collection = db[collection_name]
        print(" ---> %s..." % (collection_name))
        if "system" in collection_name:
            continue
        collection.update_many({}, {"$unset": {"_types": 1}})  # Updated from update with multi=True
        index_information = collection.index_information()
        indexes_to_drop = [key for key, value in index_information.items() if "types" in str(value)]
