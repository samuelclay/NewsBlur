...
def dashboard_graphs(request):
    ...
    return render(request, "statistics/render_statistics_graphs.xhtml", {"statistics": statistics})


def feedback_table(request):
    feedbacks = MFeedback.all()
    return render(request, "statistics/render_feedback_table.xhtml", {"feedbacks": feedbacks})
...
def revenue(request):
    ...
    content = "%s revenue: $%s<br><code>%s</code>" % (
    ...
    logging.user(
        request, "~FBGenerating Revenue RSS feed: ~FM%s" % (request.META.get("HTTP_USER_AGENT", "")[:24])
    )
