...
class Dispatcher:
    ...
    def process_feed_wrapper(self, feed_queue):
        ...
        for feed_id in feed_queue:
            ...
            try:
                ...
                logging.debug('   ---> [%-30s] Feed fetch timed out...' % (feed.title[:30]))
    ...
    def publish_to_subscribers(self, feed):
        try:
            ...
            if listeners_count:
                logging.debug("   ---> [%-30s] Published to %s subscribers" % (feed.title[:30], listeners_count))
        except redis.ConnectionError:
            logging.debug("   ***> [%-30s] ~FRRedis is unavailable for real-time." % (feed.title[:30],))
