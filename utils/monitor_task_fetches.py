...
def main():
    ...
    try:
        client = pymongo.MongoClient(
            f"mongodb://{settings.MONGO_DB['username']}:{settings.MONGO_DB['password']}@{settings.MONGO_DB['host']}/?authSource=admin"
        )
    ...
    if failed:
        requests.post(
            ...
            data={
                "from": "NewsBlur Task Monitor: %s <admin@%s.newsblur.com>" % (hostname, hostname),
                "to": [admin_email],
                "subject": "%s feeds fetched falling: %s (from %s)"
                % (hostname, feeds_fetched, redis_task_fetches),
                "text": "Feed fetches are falling: %s (from %s) %s"
                % (feeds_fetched, redis_task_fetches, failed),
            },
        )
