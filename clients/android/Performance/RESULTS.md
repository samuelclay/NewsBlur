...
**Progress report:** the current build passes 323 tests and is installed. Runtime checks across all four themes for the latest prepared internal-story navigation and folder-animation background changes, plus the final reader measurement, are still pending. The timing captures below precede those two followup fixes; this report does not mark the audit complete.
...
- A later folder-animation fix draws the themed folder surface beneath moving/fading rows, preventing temporary gaps from exposing the darker window background (`f13ce0203`). It is regression-tested and installed; runtime verification across all themes remains pending.
- Distant internal story navigation now holds the visible page snapshot while the destination viewport prepares, then animates the ready destination (`1fea8e131`). Preparation tracks pause, cancellation, current story, and navigation history. The installed implementation passes regression tests; its final reader and all-theme runtime checks remain pending.
...
- The latest combined build passed **323 tests with zero failures** (`/tmp/newsblur-prepared-navigation-full-after.log`, 43s). The APK is installed. This includes prepared internal navigation and the folder-transition background, alongside the earlier sync, cache, typography, menu, and reader fixes. Previous-button runtime verification already passed; the newest reader/folder runtime checks remain pending.
...
Verified theme captures cover the following views. They establish appearance and contrast, not a guarantee that every transition is free of flashing. They precede the newest prepared internal-navigation and folder-background fixes; those changes still need their runtime checks across all themes.
...
The far internal Next replay revealed the header about a second before the article body: `/tmp/newsblur-reader-controls-fixed-forward.png` and `/tmp/newsblur-reader-controls-fixed-forward-late.png`. The activity entrance (`9ec4d6c0c`) already had verified readiness handling. The corresponding internal-jump fix (`1fea8e131`) is now implemented, tested, and installed, but its runtime replay and final reader measurement remain pending; the earlier navigation pass does not establish flash-free far jumps.
