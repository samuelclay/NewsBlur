...
**Completed emulator audit:** the installed build passes 338 tests. Feed-list, story-title, and long-article scrolling have before/after measurements; folder transitions and prepared Next/Previous pass in all four themes. Rapid navigation, Home/resume, Back during far-page preparation, and the empty-session crash regression also pass runtime checks. Measured frame delays remain substantial, especially in the reader's emulated graphics path. This audit does not establish instantaneous loading, consistent 60fps, physical-device performance, or release readiness.
...
The recorded timings do **not** establish a reliable overall frame-rate improvement. Early UI-only measurements below precede the sync fix. Captures are observations on a shared host, with severe contention in some earlier runs; cache state, live account changes, and frame-presentation outliers are not fully controlled. The later long-body pair uses the same cached article offline to remove network/content changes from that comparison.

## Setup and reproduction

- Worktree: `android-scroll-performance`; initial source anchor `362ae7d35`, story binding `eefa8ea86`, sync serialization `82b3db2e0`, prepared reader entrance `26ea0719e`, coalesced refresh requests `a67de5799`, latest-feed query publication `0531487fe`, same-query snapshot completion `32a65b75f`, canceled reader entrance on Back `b95bb2944`, immutable page/session guard `660e8fd96`, immediate cached feed startup `193a9e17f`, serialized adapter publication `daefdc29c`, first-story anchor above the footer `1d9a4e0cd`, folder collapse scroll-clamp geometry `516ef2024`, related-story date spacing `535fe34fa`, article-body readiness before entrance `9ec4d6c0c`. Latest measured/installed source: `553b13981`, bounded folder search when no eligible session remains.
...
`final-intent/navigation-timing/` uses source `984dcc6f3`, ART speed compilation, Dark theme, and no video recording. It runs `scenarios/reader-settled.json`, which waits 3.5 seconds after each of three Next and two Previous actions before attempting two short-body swipes. The validity record retains `FeedReading`, `end-ui.txt` confirms the Lenovo article, and the reader log records navigation distances +14, +1, +2, −2, and −1. A separate normal five-control video and JSON replay (`/tmp/newsblur-reader-intent-controls.{mp4,json}`) verifies Lenovo → iPhone → Meta → iPhone → Lenovo.

| Operation | Samples | Median wall time | Mean wall time | p95 wall time | Maximum wall time | Mean scheduled CPU time |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| Choreographer frame | 305 | 9.266ms | 27.476ms | 104.301ms | 339.563ms | 2.974ms |

`gfxinfo` recorded **196 frames**, **60.20% janky**, **65ms p50**, and **300ms p95**. Prepared request-to-reveal logs include 2890ms, 1725ms, and 1441ms; these include preparation rather than measuring only the slide animation. The emulator still shows substantial presentation delays.

This run is **not a speedup comparison** with the earlier detail baseline: Previous now works, prepared animations add actual rendering work, the wait between actions is 3.5 seconds, and live article contents have changed. Its two short-body swipe attempts still do not validate long-article scrolling. The run establishes a measured, functioning navigation scenario for future matching comparisons.

## Matching long-article scrolling observation

`baseline-speed/long-body-timing/` and `final-intent/long-body-timing/` compare source `362ae7d35` with `553b13981`. Both use the same cached Daring Fireball article, **The iPhone Air and iPhone 17 Pro**, containing 15,167 characters, with Dark theme, font scale 1.0, and ART speed compilation. Wi-Fi and mobile data are disabled for both runs. Global feed state and story state are All. The Main search is `Fire`, whose broader eligible feed set avoids the baseline's empty-session crash described below. Both runs perform six 1200ms swipes, start/end at the top, and remain in `FeedReading`; their start/end screenshots visually match the article.

Times are milliseconds, **baseline → final**:

| Operation | Samples | Median wall time | Mean wall time | p95 wall time | Maximum wall time | Mean scheduled CPU time |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| Choreographer frame | 461 → 484 | 20.735 → 19.495 | 22.885 → 20.347 | 44.988 → 43.233 | 152.378 → 88.069 | 1.026 → 0.945 |

`gfxinfo` recorded **423 → 449 frames**, **44.92% → 35.41% janky**, **38 → 36ms p50**, and **89 → 73ms p95**. These are lower observed costs in one matched article/scenario pair. The shared host and uncontrolled cache state still prevent a stable percentage-speedup claim, and the remaining deadline misses do not support calling the reader consistently smooth at 60fps.

Separate videos in `baseline-speed/long-body-video/video.mp4` and `final-intent/long-body-video/video.mp4` show roughly three screens of actual article scrolling and return to the top without holes in the inspected middle/end frames. The final visual recording uses `984dcc6f3`, before the unrelated empty-session fix; quantitative final timing uses `553b13981`. Baseline video capture may overlap build preparation, so the video is used only to verify behavior. The table uses the no-video timing runs.

Final `scheduler.csv` attributes 9847.853ms of main-callback wall time to 457.239ms running, 290.725ms runnable, and 9099.888ms sleeping. RenderThread records 449 `WebViewFunctor::drawGl` calls totaling 9494.849ms and 60,166 `glGetError encode` calls totaling 5812.988ms. These nested/concurrent totals are not additive. The large difference between scheduled main-thread work and wall time again points to waits/work along the emulated WebView graphics path; a small main-thread CPU number does not establish fast frame presentation or diagnose host macOS scheduling.
...
- Distant internal story navigation now holds the visible page snapshot while the destination viewport prepares, then animates the ready destination (`1fea8e131`). Preparation tracks pause, cancellation, current story, and navigation history. Regression tests and prepared Next/Previous replay pass in all four themes, with the article body already present during the incoming transition. Background/resume and immediate Back during far-page preparation also pass.
- Rapid navigation exposed a separate race: an older asynchronous Next unread search could overwrite a newer Previous selection. The fix rejects unread-search completions after a newer navigation intent (`984dcc6f3`). Regression tests and rapid three-Next/two-Previous runtime replay pass; the log confirms an obsolete unread search was discarded.
- A filtered feed set with no eligible unread session could recurse indefinitely while looking for the next nonempty folder. This produced `StackOverflowError` on both the baseline and `984dcc6f3`. Folder traversal is now bounded and returns no target when the eligible set is exhausted (`553b13981`), preserving normal navigation without revisiting the same empty folders.
...
- The latest combined build passed **338 tests across 78 suites with zero failures, errors, or skips** plus `assembleDebug` (`/tmp/newsblur-empty-session-full-after.log`, 21s). The APK is installed and retained as `/tmp/newsblur-empty-session-fixed.apk`. This includes bounded empty-session traversal, the rapid-navigation race fix, prepared internal navigation, and the folder-transition background, alongside the earlier sync, cache, typography, menu, and reader fixes. Folder transitions and prepared Next/Previous pass in all four themes; the race fix, far-page Back cancellation, and empty-session navigation also pass runtime replay.
...
Empty-session traversal regression evidence is retained in `/tmp/newsblur-empty-session-before.log` (one failure among 16 tests) and `/tmp/newsblur-empty-session-edges-before.log` (four stack-overflow failures among 23 tests). Runtime crash logs `/tmp/newsblur-baseline-long-crash.log` and `/tmp/newsblur-current-empty-session-crash.log` show the recursive folder search on the baseline and pre-fix candidate. The corrected build opens the long Daring Fireball article with the exact narrow search and All feed/story states: `/tmp/newsblur-empty-session-runtime-after.png`. The matching crash log `/tmp/newsblur-empty-session-runtime-after.log` is empty. This is separate from the broader `Fire` search used to obtain a crash-free baseline timing run.
...
Lifecycle replay `/tmp/newsblur-reader-lifecycle.mp4` passed Home during a far 14-position preparation followed by launcher/resume, with the final Lenovo article fully painted. Its final Back action occurred during adjacent navigation; a subsequent replay explicitly tested the far-page case.

The rapid replay `/tmp/newsblur-reader-fuzz.mp4`, `/tmp/newsblur-reader-fuzz-log.txt`, and `/tmp/newsblur-reader-rapid-final.xml` passes three Next actions followed quickly by two Previous actions, ending on Anthropic. The log includes `page_search_discarded`, confirming rejection of an obsolete unread search. A new far +14 request followed by immediate Back returns to `FeedItemsList` without a late page appearing. The first after-fix attempt could not dump UI XML while the view settled; the subsequent replay with six seconds to settle passed. The original failing evidence remains under `/tmp/newsblur-reader-before-intent-*`. Corrected navigation timing and matching long-article scrolling measurements are recorded above.
...
Final emulator restoration is verified in `/tmp/newsblur-android-final-main.png`: the latest `553b13981` APK is open on Main as `samuel`, with Light theme, global `state_filter=SOME`, Engadget and Daring Fireball read filters restored to Unread, and mark read on scroll enabled. Manual story-read behavior is unchanged, `load_next_on_mark_read=false`, and row hardware layers remain enabled. Font scale and all three animation scales are 1.0; Wi-Fi/mobile data are on and emulator networking is full speed with no added delay.
...
- `final/feed-timing/` is the intermediate sync candidate described above. An earlier detail-after capture interrupted by a force-stop is excluded. Completed feed/title comparisons are `final-coalesced/feed-timing/` and `final-reviewed/titles-timing/`; both precede the latest animation/navigation fixes. `final-reviewed/detail-timing/` remains provisional because Previous was disabled in that recording. The corrected `final-intent/navigation-timing/` completes working Next/Previous but is unpaired because the scenario changed. The separate matched long-article comparison is `baseline-speed/long-body-timing/` → `final-intent/long-body-timing/`. Prepared Next/Previous and folder-background transitions pass in all four themes; Home/resume, immediate Back during far-page preparation, rapid-navigation replay, and bounded empty-session traversal also pass. The completed hardware-layer A/B/A was inconclusive and retained the enabled default. Theme appearance, offline startup, fast-scroll fuzz, and reader/folder animation captures are listed above. More repeated measurements with comparable holder/cache state would be needed for a stable speedup estimate.
