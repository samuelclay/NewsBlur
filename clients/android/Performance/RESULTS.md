...
**Progress report:** the current build passes 323 tests and is installed. Rapid folder toggles pass in all four themes, and prepared internal-story navigation passes forward, adjacent, and reverse replay in Dark. Remaining reader theme/lifecycle checks and the corrected final reader measurement are pending. The timing captures below precede those two followup fixes; this report does not mark the audit complete.
...
- Distant internal story navigation now holds the visible page snapshot while the destination viewport prepares, then animates the ready destination (`1fea8e131`). Preparation tracks pause, cancellation, current story, and navigation history. Regression tests and Dark forward/adjacent/reverse replay pass, with the article body already present during the incoming transition. Remaining reader themes, lifecycle checks, and the final reader measurement are pending.
...
- The latest combined build passed **323 tests with zero failures** (`/tmp/newsblur-prepared-navigation-full-after.log`, 43s). The APK is installed. This includes prepared internal navigation and the folder-transition background, alongside the earlier sync, cache, typography, menu, and reader fixes. Folder transitions now pass in all four themes, and prepared reader navigation passes in Dark; the remaining reader checks are described below.
...
Verified theme captures cover the following views. They establish appearance and contrast, not a guarantee that every transition is free of flashing. These appearance captures precede the newest prepared internal-navigation and folder-background fixes. The newer folder transition passes in all four themes; the prepared reader has passed Dark replay, with its remaining theme checks pending.
