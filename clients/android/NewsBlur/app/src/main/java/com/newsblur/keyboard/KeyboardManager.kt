...
class KeyboardManager {
        ...
        when (keyCode) {
            ...
            KeyEvent.KEYCODE_S -> {
                if (event.isShiftPressed) {
                    listener?.onKeyboardEvent(KeyboardEvent.ShareStory)
                } else {
                    listener?.onKeyboardEvent(KeyboardEvent.SaveUnsaveStory)
                }
                true
            }
            ...
            KeyEvent.KEYCODE_SPACE -> {
                if (event.isShiftPressed) {
                    listener?.onKeyboardEvent(KeyboardEvent.PageUp)
                } else {
                    listener?.onKeyboardEvent(KeyboardEvent.PageDown)
                }
                true
            }
    ...
    private fun handleKeycodeE(event: KeyEvent): Boolean =
    ...
    private fun handleKeycodeA(event: KeyEvent): Boolean =
        if (event.isAltPressed) {
    ...
    fun isKnownKeyCode(keyCode: Int): Boolean = isShortcutKeyCode(keyCode) && isSpecialKeyCode(keyCode)

    private fun isSpecialKeyCode(keyCode: Int) =
        when (keyCode) {
            KeyEvent.KEYCODE_DPAD_LEFT,
            KeyEvent.KEYCODE_DPAD_RIGHT,
            KeyEvent.KEYCODE_DPAD_UP,
    ...
    private fun isShortcutKeyCode(keyCode: Int) =
        when (keyCode) {
            KeyEvent.KEYCODE_E,
            KeyEvent.KEYCODE_A,
            KeyEvent.KEYCODE_J,
    ...
    companion object {
        @JvmStatic
        fun hasHardwareKeyboard(context: Context) = context.resources.configuration.keyboard == Configuration.KEYBOARD_QWERTY
    }
}
