...
class StorageModule {
    ...
    ): SharedPreferences = context.getSharedPreferences(PrefConstants.PREFERENCES, Context.MODE_PRIVATE)

    @Singleton
    @Provides
    fun provideBlurDbHelper(
        @ApplicationContext context: Context,
    ): BlurDatabaseHelper = BlurDatabaseHelper(context)

    @Singleton
    @Provides
    @StoryImageCache
    fun provideStoryCache(
        @ApplicationContext context: Context,
        @ImageOkHttpClient imageOkHttpClient: OkHttpClient,
    ): FileCache = FileCache.asStoryImageCache(context, imageOkHttpClient)

    @Singleton
    @Provides
    @IconFileCache
    fun provideIconCache(
        @ApplicationContext context: Context,
        @ImageOkHttpClient imageOkHttpClient: OkHttpClient,
    ): FileCache = FileCache.asIconCache(context, imageOkHttpClient)
