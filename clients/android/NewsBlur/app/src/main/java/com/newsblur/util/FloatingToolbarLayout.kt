...
object FloatingToolbarLayout {
    data class Fit(val merged: Boolean, val options: Int, val discoverText: Boolean, val searchText: Boolean)

    fun fit(width: Int, fullOptions: Int, compactOptions: Int, discover: Int, search: Int,
            showDiscover: Boolean = true, showSearch: Boolean = true, showMark: Boolean = true): Fit {
        ...
        val edges = if (showMark) (if (merged) 16 else 32) else 8
        val fixed = mark + gaps + edges
        val optionsSpace = width - fixed - minimumDiscover - minimumSearch
        val optionsMode = when { fullOptions <= optionsSpace -> 2; compactOptions <= optionsSpace -> 1; else -> 0 }
        val optionsWidth = when (optionsMode) { 2 -> fullOptions; 1 -> compactOptions; else -> 44 }
    ...
    data class Popover(val x: Int, val y: Int, val width: Int, val height: Int)

    fun popover(left: Int, top: Int, right: Int, bottom: Int, anchorLeft: Int, anchorTop: Int,
                anchorRight: Int, anchorBottom: Int, desiredWidth: Int, desiredHeight: Int, gap: Int): Popover {
