...
enum class DiscoveryTab(val title: String, val type: String = "all", val hint: String) {
...
data class DiscoveryFeed(
    ...
    fun asFeed(resolvedId: String = id) = Feed().also {
    ...
    companion object {
        fun parse(entry: JsonObject, autocomplete: Boolean = false): DiscoveryFeed? {
            val nested = entry.obj("feed")
            val feed = nested ?: entry
            fun first(vararg keys: String) = keys.firstNotNullOfOrNull { key ->
            ...
            val id = (nested?.string("id") ?: entry.string(if (autocomplete) "id" else "feed_id"))
                .takeIf { (it.toLongOrNull() ?: 0) > 0 }.orEmpty()
            return DiscoveryFeed(
                ...
                stories = entry.objects("stories").map { it.string("story_title").ifBlank { it.string("title") } }.filter { it.isNotBlank() },
            )
        }
    }
}

data class DiscoveryCategory(val name: String, val subcategories: List<String>)
...
data class WebVariant(val label: String, val paths: Map<String, String>, val stories: List<String>) {
    companion object {
        fun parse(json: JsonObject) = WebVariant(
            json.string("label"),
...
data class WebDiscoveryState(
    val url: String = "", val hint: String = "", val analyzedUrl: String = "", val requestId: String = "",
    val loading: Boolean = false, val message: String = "", val error: String? = null,
    val variants: List<WebVariant> = emptyList(), val selected: Int = 0,
    val title: String = "", val htmlHash: String = "", val favicon: String = "",
    val detectedFeed: String = "", val staleness: Int = 30, val markUnread: Boolean = false,
)
