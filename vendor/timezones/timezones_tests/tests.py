...
class TimeZoneTestCase(TestCase):
    ...
    def assertFormIsValid(self, form):
        ...
        self.assertTrue(is_valid, "Form did not validate (errors=%r, form=%r)" % (form._errors, form))


class UtilsTestCase(TimeZoneTestCase):
    def test_localtime_for_timezone(self):
        self.assertEqual(
            localtime_for_timezone(datetime(2008, 6, 25, 18, 0, 0), "America/Denver").strftime(
                "%m/%d/%Y %H:%M:%S"
            ),
            "06/25/2008 12:00:00",
        )
    ...
    def test_adjust_datetime_to_timezone(self):
        self.assertEqual(
            adjust_datetime_to_timezone(datetime(2008, 6, 25, 18, 0, 0), "UTC").strftime("%m/%d/%Y %H:%M:%S"),
            "06/25/2008 18:00:00",
        )
...
class TimeZoneFieldTestCase(TimeZoneTestCase):
    def test_forms_clean_required(self):
        ...
        self.assertEqual(repr(f.clean("US/Eastern")), "<DstTzInfo 'US/Eastern' EST-1 day, 19:00:00 STD>")
    ...
    def test_forms_clean_bad_value(self):
        ...
        try:
            ...
            self.assertEqual(
                e.messages, ["Select a valid choice. BAD VALUE is not one of the available choices."]
            )
...
class LocalizedDateTimeFieldTestCase(TimeZoneTestCase):
    def test_forms_clean_required(self):
        ...
        self.assertEqual(
            repr(f.clean("2008-05-30 14:30:00")), "datetime.datetime(2008, 5, 30, 14, 30, tzinfo=<UTC>)"
        )
        self.assertRaises(forms.ValidationError, f.clean, "")
