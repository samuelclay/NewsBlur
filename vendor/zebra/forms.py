...
class StripePaymentForm(CardForm):
    def __init__(self, *args, **kwargs):
        ...
        months = [(m[0], "%02d - %s" % (m[0], str(m[1]))) for m in sorted(MONTHS.items())]
    ...
    card_expiry_month = forms.ChoiceField(required=False, widget=NoNameSelect(), choices=iter(MONTHS.items()))
    card_expiry_year = forms.ChoiceField(
        required=False, widget=NoNameSelect(), choices=options.ZEBRA_CARD_YEARS_CHOICES
    )
